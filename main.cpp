...
static std::string ReadGetNameFromFName(int key)
{
	...
	uint64_t NamePoolChunk = read<uint64_t>(modules::BaseAddress + 0xC483180 + (8 * ChunkOffset) + 16) + (unsigned int)(4 * NameOffset); //((ChunkOffset + 2) * 8) ERROR_NAME_SIZE_EXCEEDED
...
// all offsets are updated to chapter 3 season 2
		offests::uworld = read<uint64_t>(modules::BaseAddress + 0xC446238);//updated 
		

		offests::game_instance = read<uint64_t>(offests::uworld + 0x190);//updated 
		...
		offests::Pawn = read<uint64_t>(offests::player_controller + 0x338);//updated 
		...
		for (int i = 0; i < Count; i++)
		{
			...
			uintptr_t CurrentPawn = read<uintptr_t>(CurrentPlayerState + 0x280);//updated 
			...
			uint64_t PlayerState = read<uint64_t>(CurrentPawn + 0x2B8);//updated 
			if (!PlayerState)continue;

			uint64_t LocalPlayerState = read<uint64_t>(offests::Pawn + 0x280);//updated 
...
void menu() {
	...
	ImGui::Begin((E("WAR UPDATED UNDETECTED")), NULL, flags);
	{
...
auto main() -> ULONG
{
	...
	//driver will be fully undetected for a lot time probaly its a very good driver
	DrverInit = CreateFileW((L"\\\\.\\brSpotify"), GENERIC_READ, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);//vulnerable undetected drivers
